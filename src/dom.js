class DOM {
  ...
  init() {
    ...
    const title = this.makeElement('span', [['class', 'title-text']], 'BATTLEWHALE');
    ...
    const blueWhale = this.makeElement('img', [['class', 'whale'],['src', './images/blue-whale.jpg']]);
    const blueWhaleText = this.makeElement('span', [['class', 'whale-text']]);
    blueWhaleText.textContent = 'Blue Whale: 5 hits';
    blueWhaleDiv.append(blueWhale, blueWhaleText);
    const rightWhaleDiv = this.makeElement('div', [['class', 'whale-div']]);


    const rightWhale = this.makeElement('img', [['class', 'whale'], ['src', './images/right-whale.jpg']]);
    const rightWhaleText = this.makeElement('span', [['class', 'whale-text']]);
    rightWhaleText.textContent = 'Right Whale: 4 hits';
    rightWhaleDiv.append(rightWhale, rightWhaleText);

    const spermWhaleDiv = this.makeElement('div', [['class', 'whale-div']]);

    const spermWhale = this.makeElement('img', [['class', 'whale'], ['src', './images/sperm-whale.jpg']]);
    const spermWhaleText = this.makeElement('span', [['class', 'whale-text']]);
    spermWhaleText.textContent = 'Sperm Whale: 3 hits';
    spermWhaleDiv.append(spermWhale, spermWhaleText);

    const humpbackDiv = this.makeElement('div', [['class', 'whale-div']]);

    const humpback = this.makeElement('img', [['class', 'whale'], ['src', './images/humpback.jpg']]);
    const humpbackText = this.makeElement('span', [['class', 'whale-text']]);
    humpbackText.textContent = 'Humpback Whale: 3 hits';

    humpbackDiv.append(humpback, humpbackText);
    const orcaDiv = this.makeElement('div', [['class', 'whale-div']]);

    const orca = this.makeElement('img', [['class', 'whale'], ['src', './images/orca.jpg']]);
    const orcaText = this.makeElement('span', [['class', 'whale-text']]);
    orcaText.textContent = 'Orca: 2 hits';
    orcaDiv.append(orca, orcaText);

    ships.append(blueWhaleDiv, rightWhaleDiv, spermWhaleDiv, humpbackDiv, orcaDiv);
    ...
    const playerSidebar = this.makeElement('div', [['class', 'player-sidebar']]);
    ...
    const copyrightLinkText = 'https://ocean.si.edu/ocean-life/marine-mammals/what-largest-whale-cetacea-size-comparison-chart';
    const copyrightLink = this.makeElement('a', [['href', 'https://ocean.si.edu/ocean-life/marine-mammals/what-largest-whale-cetacea-size-comparison-chart']], copyrightLinkText);
    const copyrightText = 'Whale images are used subject to usage conditions from the Smithsonian Institution:';    
    const copyTextDiv = this.makeElement('div', [['class', 'copy-text']], copyrightText);
